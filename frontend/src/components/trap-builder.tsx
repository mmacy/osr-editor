...
// never asked for; where the trap lives decides it. A room trap's trigger is
// authorable: osrlib springs "enter" when the party steps onto a cell of the
// area and "open" when a door of the area swings, either side — the
// blade-drops-when-the-door-swings construction — so both values are live and
// the builder offers the choice with each value's springing action named. A
// cache trap shows no trigger field at all: the cache-open path springs it
// unconditionally, and TrapSpec rejects a treasure trap carrying "enter" at
// parse. Every field change emits a patch through onPatch/onEffectPatch; the
// owner merges it against the committed trap inside the store's queue
// (patchTrapEffect holding the validators' implications: volley only with
// damage, duration only with a condition), so a queued edit never reverts an
// in-flight one.
    ...
    <div className="flex flex-col gap-3" aria-label="Trap">
      ...
      {trap.kind === 'room' && (
        <p className="text-muted-foreground text-xs">
          {trap.trigger === 'enter'
            ? 'Springs when the party steps onto a cell of the area.'
            : 'Springs when a door of the area is opened, from either side.'}
        </p>
      )}
