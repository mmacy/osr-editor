...
// follow).
import { copyFileSync, cpSync, lstatSync, mkdirSync, mkdtempSync, readFileSync, realpathSync } from 'node:fs'
...
test('the content-library milestone: stock an OPD level from sources, swap the map, re-place from the stash', async ({
  ...
  await expect(
    forgePack.getByTestId(`entry-dungeon:millstone-warrens/level:1/area:2`).getByTitle(
  ...
  await expect(
    forgePack.getByTestId(`entry-dungeon:millstone-warrens/level:2/area:1`).getByTitle(
      'Placed at least once',
    ),
  ).toBeVisible()
  ...
  await expect(
    millPack.getByTestId(`entry-dungeon:mill-caves/level:2/area:4`),
  ).toBeVisible()
  ...
  await expect(
    offer.getByRole('checkbox', { name: "Stash this level's content in the library first" }),
  ).toBeChecked()
  ...
  const stashCollision = page.getByRole('dialog', { name: 'The room already has some of this' })
  ...
  await expect(
    stashPack.getByTestId(`entry-dungeon:dungeon-1/level:2/area:2`).getByTitle(
      'Placed at least once',
    ),
  ).toBeVisible()
