...
test("the content-library milestone: stock an OPD level from sources, swap the map, re-place from the stash", async ({
  ...
  await stashRow
    .getByRole("button", { name: /^Open dungeon-1 level 2/ })
    .click();
  ...
  expect(document.payload.monsters.map((monster) => monster.id)).toContain(
    "mill_wisp",
  );
  const level2 = document.payload.dungeons[0].levels.find(
  ...
  expect(landed.encounter?.monsters.map((line) => line.template_id)).toEqual([
    "goblin",
  ]);
  ...
  const sidecar = JSON.parse(
    readFileSync(join(projectDir, "editor.json"), "utf-8"),
  ) as {
