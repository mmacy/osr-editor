...
`release.yml`, the docs job, and the tag-driven pipeline are phase 10's, matching the siblings' release engineering.
...
- **`tests/e2e/tsconfig.json` exists to resolve imports — and is checked.** Playwright specs in the spec-pinned `tests/e2e/` location sit outside `frontend/`, so node resolution cannot find `@playwright/test`; a minimal tsconfig with a `paths` mapping (a mechanism Playwright documents) points the import at `frontend/node_modules`. It is resolution plumbing, not a second toolchain. Per rubber-duck review, the spec imports `StatusResponse` from the frontend's generated-type aliases (type-only, erased at runtime) instead of hand-casting a shape, and the e2e CI job runs `tsc -p ../tests/e2e` so the spec is type-checked, not just transpiled.
- **A note for phase 1's planner:** `LocalProjectStore.write_artifact` reads the process umask via the momentary `os.umask(0)`/restore dance to give artifacts the default file mode. The window is a few instructions wide and phase 0 has no concurrent writers, but phase 1 introduces per-project locks and real request concurrency around store writes — cache the umask once at module import or store construction if this ever matters. Relatedly, `list_artifacts` returns empty for a nonexistent project directory; phase 1's open-by-path flow needs to distinguish a typo'd path from an empty project and should decide that contract deliberately (an existence probe on the protocol, or a changed return).
