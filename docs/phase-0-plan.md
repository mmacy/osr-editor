...
- The script then runs the pinned openapi-typescript from `frontend/` (`npm exec`) and writes the single committed artifact: `frontend/src/types/generated/api.ts`. The intermediate OpenAPI document is not committed — one generated artifact, one drift gate. Friendly aliases (`export type Adventure = components["schemas"]["Adventure"]`) live in a tiny hand-written `frontend/src/types/index.ts` — type-checked, so a vanished schema breaks loudly at `tsc` time, and never hand-mirroring a shape (the guide's fence).
...
- **app**: status shape; auth-dependency route introspection; error envelope for a mapped osrlib error; static mount serves the built `index.html` when present.
- **frontend (vitest)**: status component renders fetched data; typed client compiles against generated types.
