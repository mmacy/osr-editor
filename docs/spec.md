...
**Everything the library opens is a content pack.** A pack is a set of entries — geometry-free room projections, each carrying a stable entry id and any subset of name, description, encounter, trap, treasure, and features — plus the bundled monster templates those entries reference and optional wandering tables. The pack is an osrlib-owned document kind: osrlib grows `content_pack` as an additive release with the editor as its first consumer — the SRD-stocking precedent — owning the entry and `ContentPack` models, a `validate_content_pack` beside `validate_adventure`, and, decisively, migration. A pack persists osrlib model dumps, and shared packs are the family's longest-lived artifacts, so they must ride the same `check_document`-and-migrate machinery every other stamped kind rides rather than rot outside it; the editor owns every surface a user touches. The entry is also where unplaced content legitimately lives: an `AreaSpec` requires cells by design, and the pack keeps that invariant out of the document by holding placeless rooms itself. Packs have three habitats, and the panel, drops, closure, and provenance never care which one a pack came from:

- **Derived** — opening a project (native or forge workdir, the currently open project included) projects its document into pack shape in memory, loaded and validated through the `ProjectStore` seam: a read, not a second project session, and the source is never written. Entry ids are the source areas' addresses. The `GeometryImporter` seam stays geometry-only by design; the library is its content-bearing counterpart, not an extension of it.
- **Embedded** — the displaced-content stash is a stamped pack stored in the sidecar (below), migrating on open like any osrlib document.
...
- **Provenance is a copy record, never a link.** Each drop records target-area address → pack identity and entry id in the sidecar, powering used/unused badges on the palette — stocking is a checklist activity and rarely one sitting. Re-dropping is allowed (used means at least once), nothing ever re-syncs, and a pack that has since moved or vanished degrades to badges only: the copies are already complete.
- **Displaced content feeds the library.** The two bulk content-destroying acts — a replace-mode geometry import and clear level content — offer to move the level's rooms and wandering table into the sidecar stash before the batch applies, and the stash appears in the panel as a pack of its own. This is the swap-in workflow: replace a level's map with an OPD import, keep its content in the library, re-place it room by room. Stash entries are deleted manually when restocking is done, with a fully-used state making that moment obvious; undoing the replace restores the level while the stash entry persists — a harmless duplicate, not a conflict, and deletable. The stash is the one sidecar field that carries content rather than metadata, an owner-sanctioned posture change within the additive-only schema.

**Standalone packs (phase 12)** are the sharing surface — the DLC model: a friend's bespoke monsters, traps, and treasures stocking your dungeon. A pack file is `stamp_document("content_pack", …)`, vetted on load with `check_document` — the ordinary family envelope, riding osrlib's schema counter and migrating on open exactly as adventure documents do. Save the stash or any selection of entries as a pack (closure computed, metadata — name, description, author — prompted); open a pack file in the panel like any other source; a monsters-only pack lists its templates for one-click `AddMonsterTemplate` under the same reuse-or-re-mint rule. Saving a pack is the same explicit user-invoked export the licensing fence already allows: a pack cut from a converted module is the user's to have, and what they share is their responsibility — exactly as it is when publishing the conversion itself.
...
- **Backend unit tests (pytest):** op application and rejection per invariant, undo/redo, canonical-serialization byte-stability (write → reopen → no-op → write, byte-identical, for documents the editor wrote under the pinned osrlib), lint findings against fixture adventures (asserting parity with forge's finding ids on shared cases), op→override translation goldens, override reason drafting, content-library drop goldens (carried-kind ops, collision handling, template closure and re-minting, stash and pack round-trips), publish gating.
