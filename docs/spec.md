...
- **Features** — treasure caches, construction tricks, and custom features, bound to the whole area or a specific cell. The contents — item ids picked from the equipment catalog, coins and named valuables on forms mirroring `Coins` and `ValuableSpec` — are offered on treasure caches only, because osrlib's take and inspect handlers reject any feature of another kind, so contents elsewhere are unreachable in play. Leaving the cache kind clears the contents with the trap in the kind change's own batch — one undo step, and undo is the way back. A non-cache feature that *arrives* carrying contents keeps them (an incoming document is never silently rewritten) and warns, naming them, since the gate would otherwise hide contents that still serialize and still publish; the kind select is the recovery.
- **Traps** — a builder over `TrapSpec`/`TrapEffect`: room traps on areas, treasure traps on caches (the split is enforced by the models), affects, and the composable effect fields — damage, volley, save, condition + duration, fall, slide transition, and the `manual` prose fallback for what B/X leaves to the referee. The trigger follows the kind rather than being asked for: osrlib springs a room trap on `enter` alone and never reads a cache trap's trigger, so a room trap shows its trigger as read-only text and a treasure trap shows none. The readout is the document's value, never a comfortable fiction — a room trap carrying any other trigger warns that it never springs and offers a one-patch repair, so removing the control strands nobody.
...
**Quest milestones are budget line items** — a name and an XP value, badged as referee-awarded. Authoring them into the document is the quest surface's future work (see future extensions); budgeting them is not blocked by it.
...
- **Quest authoring.** osrlib 1.5 grew the model this entry once waited on: gates on door and transition `requires`, trigger wiring on `Adventure.triggers`, and `Adventure.quests` — activation and objectives, hidden/reveal clauses, and rewards issued as referee commands, `AwardXP` among them. The growth path stands exactly as designed — osrlib owns the schema, the generated types pick it up, the ops vocabulary grows additively, triggers place on the map the way transitions do, and diagnostics lint dangling quest references the way they lint monster ids — and becomes its own spec amendment and phase when the owner schedules it. Until the pinned floor reaches the models, the open-fields fidelity guard (see documents and versioning) keeps the editor from silently eating a quest-bearing document; the planner budgets quest XP as line items either way, and the audit counts authored rewards from the floor raise onward (see the adventure planner).
