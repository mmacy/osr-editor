...
- Additive osrlib growth *within* a schema version is guarded too: on open, the editor re-serializes the parsed payload and diffs it against the source. Any payload field the pinned models didn't capture triggers a prominent warning before the first write — an always-saved editor would otherwise silently drop fields a newer osrlib authored — with the upgrade remedy named.
- Native-project serialization is canonical — pydantic field order, 2-space indent, `ensure_ascii=False`, trailing newline — so a no-op session produces no diff and projects live happily in git.
...
- The form covers the whole model: AC pair (with the attack-roll-required coupling the validator enforces), hit dice (count/die/modifier/asterisks or fixed hp), attack routines built as ordered lists of attacks (count, name, damage dice or fixed, by-weapon, effect tags), movement modes, the five saves plus save-as, morale with alternates, alignment options, XP with role notes, number appearing (dungeon/lair), treasure refs, abilities (tag, name, prose, `manual` flag, params), defenses (harmed-only-by, reductions, energy defenses, condition immunities), and categories.
- Bundled templates live on `Adventure.monsters`; the id-collision rule (never shadow the shipped catalog) is enforced as a model-tier rejection with a rename prompt.
- In forge-backed projects the same form edits raw extracted stat blocks as `monster_templates:` override patches, with the report's `derived` field list badged on every field the mapping derived rather than read — the review workflow forge's phase 7 built the data for.
...
- **Map correction is the flagship case.** Forge's own spec expects synthesized geometry not to match the printed map; redrawing rooms, walls, doors, and transitions on the graph paper — or importing a level drawn in a scratch project — writes `geometry:` entries, no detach required.
- **Reasons** — every override entry carries a `reason`. The editor auto-drafts one from the op ("room 7 cells redrawn to match p. 14") and the corrections panel lists every entry with its reason editable, because the file remains the human-readable, git-reviewable record forge designed it to be.
...
- Ops with no override expression — `WanderingSpec` changes, adding a dungeon or level, bundling a monster the module never mentions — are blocked in place with the detach offer.
...
- On confirm, `convert()` runs in a subprocess with `on_progress` streamed to a progress view — stage transitions, token usage, per-stage status. A kill keeps completed stages (forge's own resume semantics); `rerun` picks up where it stopped. On success the project opens directly into the review queue.
...
- **Backend unit tests (pytest):** op application and rejection per invariant, undo/redo, canonical-serialization byte-stability (open → no-op → identical bytes), lint findings against fixture adventures (asserting parity with forge's finding ids on shared cases), op→override translation goldens, override reason drafting, publish gating.
- **Round-trip fixtures:** the B3 document (osr-web's bundled adventure) and a doors-and-transitions document (dcc81-shaped) as committed fixtures — open, edit, undo, save, validate.
